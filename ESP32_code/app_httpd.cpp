...
#define MAX_EFFECT 30  // max ramp-up
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  } else if (!strcmp(variable, "m1")) {
     m1 = val;
  } else if (!strcmp(variable, "m2")) {
     m2 = val;
  } else if (!strcmp(variable, "m3")) {
     m3 = val;
  } else if (!strcmp(variable, "m4")) {
     m4 = val;
  }
...
static esp_err_t status_handler(httpd_req_t *req) {
  ...
  *p++ = '{';
  ...
  p += sprintf(p, "\"gyro\":{\"x\":%.2f,\"y\":%.2f,\"z\":%.2f},", gx, gy, gz);
  p += sprintf(p, "\"accel\":{\"x\":%.2f,\"y\":%.2f,\"z\":%.2f},", ax, ay, az);
  p += sprintf(p, "\"temp\":%.2f,", tempC);
